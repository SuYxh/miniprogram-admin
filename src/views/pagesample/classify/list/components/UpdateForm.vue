<template>
  ...
  >
    <template #footer>
      ...
      <a-button key="submit" type="primary" :loading="onSubmitLoading" @click="onFinish"
        >提交</a-button
      >
    </template>
    ...
    <a-form :labelCol="{ span: 4 }" :wrapper-col="{ span: 20 }">
      <a-form-item label="名称" v-bind="validateInfos.title">
        <a-input v-model:value="modelRef.title" placeholder="请输入名称" />
      </a-form-item>
      <a-form-item label="是否可见" v-bind="validateInfos.visible">
        <TypeSelect v-model:value="modelRef.visible" placeholder="请选择" />
      </a-form-item>

      <a-form-item label="图标" v-bind="validateInfos.icon">
        <a-input v-model:value="modelRef.icon" placeholder="请输入网址" />
      </a-form-item>
    ...
    props: {
      ...
      onSubmit: {
        type: Function as PropType<
          (values: TableListItem, resetFields: (newValues?: Props | undefined) => void) => void
        >,
        required: true,
      },
    },
